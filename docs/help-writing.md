...
The contract for anything that goes into the in-app help site at `/help`.

**This file is the single source of truth, and both agents use it.** Claude Code
reaches it through the `help-writer` skill in `.claude/skills/`, which does
nothing but send you here. Codex reads it directly — `AGENTS.md` points at this
path. Change the guidance here and both agents get it; there is no second copy
to keep in step.

See [help-site.md](help-site.md) for the site's architecture, route table,
current coverage, and known gaps. This file covers only *how to write*.
