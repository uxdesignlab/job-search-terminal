...
| `scope_status` | Legacy maintenance label (migration `0029`), formerly `out_of_scope` when a verified active posting no longer matched saved title filters. **No longer written** — the out-of-scope shortcut was removed with the 0.17.0 cleanup rewrite, and its writer was deleted in 0.17.2. Existing values are retained and still read into `JobRecord.scopeStatus`, but nothing sets or displays them |
...
| `evidence_audit_json` | Unsupported-claim audit details, plus a `reverted` array naming each section whose AI rewrite was replaced with source wording and the claims that caused it, a `restored` array naming each line kept at its source wording to preserve a job phrase the rewrite would have dropped, an `unchanged` array counting selected sections the model handed back as written, a `checks` array (`{ id, label, status: "pass" \| "flag", detail }`, the *ATS & recruiter checks* report from `checkResume` at generation time), and a `unitFailures` array (`{ unit, label, reason }`, one per part the unit writer could not produce — `unit` is `role:<index>`, `impact`, `skills`, `summary`, or `extra:<id>`). Drafts saved before these existed lack the keys |
...
| `generation_stages_json` | Array of `GenerationStageTiming`: `{ stage: "preparing" \| "writing" \| "checking" \| "saving", ms, detail?, provider?, model? }`. `detail` is `reused`, `generated`, or `unavailable` for preparation, and `N of M parts` for writing |
| `created_at` | ISO timestamp |
...
One row per completed **Settings → Scan sources → Validate sources** pass. Added in migration
...
| `compensation_research_status` | `not_run` / `completed` / `unavailable` / `failed`. `not_run` also covers a lookup still in progress: preparation saves without waiting more than `COMPENSATION_WAIT_MS` for it, and the lookup rewrites only the compensation columns when it finishes (`updateApplicationPreparationCompensation`), provided the row still has the same hashes and is still `not_run`. The saved salary answer says research had not finished while it is pending |
