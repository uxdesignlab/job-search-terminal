...
Returning users go back to the README but don't find a "just run `npm run dev`"
shortcut. The Quick Start section covers first-time setup; the Core Commands table
buries `npm run dev` among database and build commands.

**Fix direction:** Add a short "Day-to-day usage" or "Starting the app again" section
near the top of the README, immediately after Quick Start.
...
The README badge says "local-first" and the opening paragraph says "all on your own
computer," which a user reasonably read as implying a local LLM (Ollama / LM Studio).
The Privacy section does clarify the AI-provider data flow, but it's several screens
down.

**Fix direction:** Add one sentence near the top of the README (and on the marketing
site) that makes the data flow explicit — e.g. "Your data stays on your machine. AI
features send text to the provider you configure (OpenAI, Anthropic, or Gemini) — no
